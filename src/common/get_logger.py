...
def log_file(LEVEL):
    ...
    base_dir = os.path.dirname(PublicEngineConfig.LOGGING_FILE_PATH)
    if not os.path.exists(base_dir):
        os.makedirs(base_dir)

    log_path = PublicEngineConfig.LOGGING_FILE_PATH
    file_log_handler = MyTimedRotatingFileHandler(filename=log_path, when="midnight", interval=1, backupCount=2)
    ...
    # file_log_handler.extMatch = re.compile(r"^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}.log$")
